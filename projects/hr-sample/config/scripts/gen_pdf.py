...
def half_report(y, h):
    ...
    # 보정회의는 2025-02-20에야 시범 도입됐다. 2024년 차수에는 제도화 이전의
    # 비정형 조정만 있었으므로 절 제목과 서술을 시기에 맞게 바꾼다.
    formalized = (y, h) >= (2025, 1)
    f.append(P("3. 보정회의 결과" if formalized else "3. 1차·2차 평가 간 등급 조정", "h2"))
    if not formalized:
        f.append(P("이 차수는 보정회의가 제도화되기 전이다(시범 도입 2025-02-20, 의무화 "
                   "2026-01-15). 아래 수치는 2차 평가자가 개별적으로 조정한 건수를 "
                   "사후 집계한 것으로, 조정 사유 기록 의무는 아직 없었다."))
    f.append(K.table([
        ["구분", "건수", "비율"],
    ...
    f.append(Spacer(1, 6))
    f.append(P("%s 등급이 변경된 건은 총 %d건(%s)이다. 하향이 상향보다 많은 구조는 "
               "2024년 첫 차수부터 매 차수 반복되고 있으며, 이번 차수도 예외가 아니었다."
               % ("보정으로" if formalized else "조정으로",
